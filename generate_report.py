...
def generate_report(nmap_results, nikto_results, report_filepath):
    ...
    # content.append(
    #     Paragraph(
    #         f"Scan Start Time: {datetime.now().strftime('%B %d, %Y %H:%M:%S %Z')}",
    #         normal_style,
    #     )
    # )
    # content.append(
    #     Paragraph(
    #         f"Scan End Time: {datetime.now().strftime('%B %d, %Y %H:%M:%S %Z')}",
    #         normal_style,
    #     )
    # )
    ...
    # # Recommendations Section (only if there are relevant open ports)
    # if open_ports:
    #     content.append(Paragraph("4. Recommendations", heading_style))

    #     # Dynamically add recommendations for open ports
    #     for port, service in open_ports:
    #         if "mysql" in service.lower():
    #             content.append(
    #                 Paragraph(
    #                     f"1. Secure MySQL service on port {port}/tcp:", normal_style
    #                 )
    #             )
    #             content.append(
    #                 Paragraph(
    #                     "   - Apply the latest patches and ensure secure configurations.",
    #                     normal_style,
    #                 )
    #             )
    #         else:
    #             content.append(
    #                 Paragraph(
    #                     f"1. Review security configuration for {service} on port {port}/tcp:",
    #                     normal_style,
    #                 )
    #             )
    #             content.append(
    #                 Paragraph(
    #                     f"   - Ensure the {service} service is properly secured and up-to-date.",
    #                     normal_style,
    #                 )
    #             )

    #     # Additional recommendations
    #     content.append(Paragraph("2. Regular Updates and Patching:", normal_style))
    #     content.append(
    #         Paragraph(
    #             "   - Keep all services, including web servers, updated with the latest patches.",
    #             normal_style,
    #         )
    #     )
